...
const browserPanActions = createSelector(imageOverflow, (imageOverflow) => {
  ...
  const browserPanX =
    !imageOverflow.left && !imageOverflow.right
      ...
      ? "pan-right"
      : "";
  const browserPanY = !imageOverflow.top && !imageOverflow.bottom ? "pan-y" : !imageOverflow.top ? "pan-up" : !imageOverflow.bottom ? "pan-down" : "";
...
export default class PinchZoomWithPins extends React.Component {
  state = {};
  ...
  maybeHandleDimensionsChanged() {
    if (this.isImageReady) {
      ...
      if (
        !isEqualDimensions(containerDimensions, getDimensions(this.state.containerDimensions)) ||
  ...
  getCorrectedTransform(requestedTransform, tolerance) {
    ...
    const top = overflow.height
      ? constrain(negate(overflow.height) * upperBoundFactor, overflow.height * upperBoundFactor - overflow.height, requestedTransform.top)
      : position === "center"
      ? (containerDimensions.height - imageDimensions.height * scale) / 2
      : initialTop || 0;

    const left = overflow.width
      ? constrain(negate(overflow.width) * upperBoundFactor, overflow.width * upperBoundFactor - overflow.width, requestedTransform.left)
      : position === "center"
  ...
  applyInitialTransform(speed = 0) {
    ...
    const scale = String(initialScale).toLowerCase() === "auto" ? getAutofitScale(containerDimensions, imageDimensions) : initialScale;
  ...
  render() {
    ...
    const { zoomButtons, maxScale, pins, callback, PinStyle } = this.props;
    const { scale } = this.state;

    const touchAction = this.controlOverscrollViaCss ? browserPanActions(this.state) || "none" : undefined;
    ...
    return (
      <div style={containerStyle}>
        ...
        {pins.map((pin) => (
          <PinStyle
            onClick={() => callback(pin)}
            style={{
              position: "absolute",
              top: this.state.top + pin.top * this.state.scale,
              left: this.state.left + pin.left * this.state.scale,
            }}
          >
            {pin.label}
          </PinStyle>
...
PinchZoomWithPins.defaultProps = {
  initialScale: "auto",
  minScale: "auto",
  maxScale: 1,
  position: "center",
...
PinchZoomWithPins.propTypes = {
  ...
  PinStyle: PropTypes.elementType,
};
